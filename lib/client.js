...
function Client(options) {
    ...
    self.topicMetadata = Object.create(null); // null prototype: topic names are data-driven

    self.correlationId = 0;

    // group metadata
    self.groupCoordinators = Object.create(null);

    // broker rack info (nodeId -> rack string)
    self.brokerRacks = {};

    // topic ID cache (KIP-516)
    self.topicIds = Object.create(null);    // topicName -> UUID string
    self.topicNames = Object.create(null);  // UUID string -> topicName

    // idempotent/transactional state
    self.producerId = -1;
    self.producerEpoch = -1;
    self.transactionCoordinators = Object.create(null);

    // share group coordinators (KIP-932)
    self.shareCoordinators = Object.create(null);
...
Client.prototype.init = function () {
    ...
    if (self.options.ssl.ca && !/^-----BEGIN CERTIFICATE-----/.test(self.options.ssl.ca.toString('utf8'))) {
        // chain after the cert/key reads — overwriting p would drop their promise
        p = p.then(function () {
            return readFile(self.options.ssl.ca).then(function (ca) {
...
Client.prototype.end = function () {
    ...
    return Promise.all(
        Array.prototype.concat(
            self.initialBrokers, Object.values(self.brokerConnections),
            Object.values(self.groupCoordinators), Object.values(self.transactionCoordinators),
            Object.values(self.shareCoordinators))
        .map(function (c) {
...
Client.prototype.parseHostString = function (hostString) {
    ...
    return {
        ...
        port: parsed.port ? parseInt(parsed.port) : 9092
    };
};
...
Client.prototype.updateMetadata = function (topicNames) {
    var self = this;
    var runningTopics, uncovered;

    if (self._updateMetadata_pending) {
        // if the in-flight refresh doesn't cover the requested topics, chain a
        // follow-up — otherwise this caller would get back metadata without them
        runningTopics = self._updateMetadata_topics || [];
        uncovered = (topicNames || []).some(function (t) {
            return runningTopics.indexOf(t) === -1;
        });
        if (uncovered) {
            return self._updateMetadata_running.catch(function () { return null; }).then(function () {
                return self.updateMetadata(topicNames);
            });
        }
        return self._updateMetadata_running;
    }

    self._updateMetadata_topics = topicNames;
...
Client.prototype._sendToAnyBroker = function (correlationId, buffer, parseResponse) {
    ...
    // candidates: initial (bootstrap) brokers plus any live broker connections —
    // a long-running client must survive its bootstrap brokers being decommissioned
    self.initialBrokers.concat(Object.values(self.brokerConnections)).forEach(function (c) {
        var key = c.server();
        if (!seen[key]) {
            seen[key] = true;
            candidates.push(c);
    ...
    if (total === 0) {
        return Promise.reject(new errors.NoKafkaConnectionError('all', 'No brokers available'));
    }
    ...
    return self._waitMetadata().then(function () {
        ...
        return Promise.all(compressionPromises).then(function () {
            return Promise.all(Object.keys(requests).map(function (leader) {
                ...
                // a concurrent metadata refresh may have removed this leader
                if (!conn) {
            ...
            return Promise.all(_mapTopics(topics).map(function (r) {
                ...
                return Promise.all(messageSet.map(function (m) {
                    ...
                    return self._decompressMessageSet(m.message).catch(function (err) {
                        ...
                        return []; // flattened away — undefined would crash the consumer loop
                    });
                }))
...
Client.prototype.offsetCommitRequestV0 = function (groupId, requests) {
    ...
    return self._waitMetadata().then(function () {
        return Promise.all(Object.keys(requests).map(function (leader) {
            ...
            if (!self.brokerConnections[leader]) {
                return _fakeTopicsErrorResponse(topics, errors.byName('LeaderNotAvailable'));
            }

            buffer = self.protocol.write().OffsetCommitRequestV0({
                correlationId: correlationId,
                clientId: self.options.clientId,
                groupId: groupId,
                topics: topics
...
Client.prototype.updateGroupCoordinator = function (groupId) {
    var self = this;
    var cached = self.groupCoordinators[groupId];

    // remove from the cache synchronously so a concurrent _findGroupCoordinator
    // can't be handed the known-bad coordinator
    delete self.groupCoordinators[groupId];

    if (cached) {
        return cached.then(function (connection) {
            connection.close();
        })
        .catch(function (err) {
...
Client.prototype.updateTransactionCoordinator = function (transactionalId) {
    var self = this;
    var cached = self.transactionCoordinators[transactionalId];
...
Client.prototype.apiVersionsRequest = function (connection) {
    ...
    .catch(function (err) {
        if (err instanceof errors.NoKafkaConnectionError) {
            ...
            if (!/closed connection/i.test(err.message)) {
                // pure transport failure (refused/timeout) — leave null so version
                // discovery is retried on the next metadata refresh instead of
                // permanently pinning this broker to v0 protocol
                connection.apiVersions = null;
                return null;
            }
        }
        // pre-0.10 brokers close the connection on ApiVersions — genuine v0 era
...
Client.prototype._findTransactionCoordinator = function (transactionalId) {
    ...
    if (coordMax >= 4) {
        ...
        return Promise.reject(new Error('Transactions require FindCoordinator v1+ (coordinatorType support). Broker only supports v0.'));
    }
    ...
    return self._findTransactionCoordinator(transactionalId).then(function (connection) {
        ...
        return connection.send(correlationId, buffer).then(function (responseBuffer) {
            ...
            if (txnVersion >= 5 && result.producerEpoch !== undefined) {
                // Int64BE decodes to a Long — keep producerId a plain number
                self.producerId = (result.producerId && typeof result.producerId === 'object' && result.producerId.toNumber)
                    ? result.producerId.toNumber()
                    : result.producerId;
...
Client.prototype._applyLeaderHints = function (topics, nodeEndpoints) {
    ...
    // Wait for API version discovery on new connections before returning
...
Client.prototype.updateShareCoordinator = function (groupId) {
    var self = this;
    var cached = self.shareCoordinators[groupId];
...
Client.prototype.shareGroupHeartbeatRequest = function (groupId, memberId, memberEpoch, rackId, subscribedTopicNames) {
    ...
    return self._findGroupCoordinator(groupId).then(function (connection) {
        var correlationId, buffer;

        if (connection.apiVersions && !connection.apiVersions[globals.API_KEYS.ShareGroupHeartbeatRequest]) {
            throw new Error('Broker ' + connection.server() + ' does not support ShareGroupHeartbeat'
                + ' (share groups require Kafka 4.0+ with KIP-932 enabled)');
        }

        correlationId = self.nextCorrelationId();
        buffer = self.protocol.write().ShareGroupHeartbeatRequestV1({
            correlationId: correlationId,
            clientId: self.options.clientId,
            groupId: groupId,
            memberId: memberId || '',
...
// KIP-848: next-gen consumer group rebalance protocol.
// subscribedTopicNames / serverAssignor / topicPartitions may be null to mean "unchanged".
Client.prototype.consumerGroupHeartbeatRequest = function (groupId, memberId, memberEpoch, subscribedTopicNames, serverAssignor, topicPartitions, rebalanceTimeoutMs) {
    var self = this;

    return self._findGroupCoordinator(groupId).then(function (connection) {
        var correlationId, buffer;

        if (connection.apiVersions && !connection.apiVersions[globals.API_KEYS.ConsumerGroupHeartbeatRequest]) {
            throw new Error('Broker ' + connection.server() + ' does not support ConsumerGroupHeartbeat'
                + ' (the KIP-848 consumer protocol requires Kafka 4.0+)');
        }

        correlationId = self.nextCorrelationId();
        buffer = self.protocol.write().ConsumerGroupHeartbeatRequestV1({
            correlationId: correlationId,
            clientId: self.options.clientId,
            groupId: groupId,
            memberId: memberId || '',
            memberEpoch: memberEpoch,
            instanceId: self.options.groupInstanceId || null,
            rackId: self.options.rackId || null,
            rebalanceTimeoutMs: rebalanceTimeoutMs !== undefined ? rebalanceTimeoutMs : -1,
            subscribedTopicNames: subscribedTopicNames || null,
            subscribedTopicRegex: null,
            serverAssignor: serverAssignor || null,
            topicPartitions: topicPartitions || null
        }).result;

        return connection.send(correlationId, buffer).then(function (responseBuffer) {
            var result = self.protocol.read(responseBuffer).ConsumerGroupHeartbeatResponseV1().result;
            if (result.error) {
                throw result.error;
            }
            return result;
...
Client.prototype.shareFetchRequest = function (groupId, memberId, shareSessionEpoch, requests, forgottenTopics, acknowledgements) {
    ...
    return self._waitMetadata().then(function () {
        return Promise.all(Object.keys(requests).map(function (leader) {
            ...
            // share sessions are per-broker: accept either a single epoch (number)
            // or a per-leader map { leaderId: epoch }
            epoch = (shareSessionEpoch !== null && typeof shareSessionEpoch === 'object')
                ? (shareSessionEpoch[leader] || 0)
                : shareSessionEpoch;
