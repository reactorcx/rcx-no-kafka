...
Client.prototype.metadataRequest = function (topicNames) {
    ...
    // check if any initial broker has version info for metadata v1
    var canUseV1 = _.some(self.initialBrokers, function (conn) {
        return conn.apiVersions && conn.apiVersions[globals.API_KEYS.MetadataRequest] &&
            conn.apiVersions[globals.API_KEYS.MetadataRequest].max >= 1;
    });
...
Client.prototype.produceRequest = function (requests, codec) {
    ...
    function processPartition(pv, pk, leader) {
        ...
        var produceVersion = conn ? self._negotiateVersion(conn, globals.API_KEYS.ProduceRequest, 3) : 0;
...
Client.prototype.fetchRequest = function (requests) {
    ...
    return self._waitMetadata().then(function () {
        return Promise.all(_.map(requests, function (topics, leader) {
            ...
            fetchVersion = self._negotiateVersion(conn, globals.API_KEYS.FetchRequest, 4);
...
Client.prototype.getPartitionOffset = function (leader, topic, partition, time) {
    ...
    var request = {};
    var conn = self.brokerConnections[leader];
    var offsetVersion = conn ? self._negotiateVersion(conn, globals.API_KEYS.OffsetRequest, 1) : 0;
