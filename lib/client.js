...
Client.prototype.end = function () {
    ...
    return Promise.all(
        Array.prototype.concat(self.initialBrokers, Object.values(self.brokerConnections), Object.values(self.groupCoordinators), Object.values(self.transactionCoordinators))
        .map(function (c) {
...
Client.prototype.produceRequest = function (requests, codec, compressionLevel) {
    ...
    function processPartition(pv, pk, leader) {
        ...
        var produceVersion = conn ? self._negotiateVersion(conn, globals.API_KEYS.ProduceRequest, 11) : 0;
    ...
    return self._waitMetadata().then(function () {
        var leaderGroups = Object.groupBy(requests, function (r) { return r.leader; });
        requests = {};
        Object.keys(leaderGroups).forEach(function (leader) {
            var topicGroups = Object.groupBy(leaderGroups[leader], function (r) { return r.topic; });
            requests[leader] = Object.keys(topicGroups).map(function (t) {
                var partGroups = Object.groupBy(topicGroups[t], function (r) { return r.partition; });
                return {
                    topicName: t,
...
Client.prototype.offsetRequest = function (requests) {
    ...
    return self._waitMetadata().then(function () {
        return Promise.all(Object.keys(requests).map(function (leader) {
            ...
            var correlationId = self.nextCorrelationId();
            var conn = self.brokerConnections[leader];
            var offsetVersion = conn ? self._negotiateVersion(conn, globals.API_KEYS.OffsetRequest, 9) : 0;
            var buffer, responseName;
...
Client.prototype.offsetFetchRequestV1 = function (groupId, requests) {
    ...
    return self._waitMetadata().then(function () {
        return self._findGroupCoordinator(groupId).then(function (connection) {
            var correlationId = self.nextCorrelationId();
            var fetchVersion = self._negotiateVersion(connection, globals.API_KEYS.OffsetFetchRequest, 9);
    ...
    self.groupCoordinators[groupId] = Promise.any(self.initialBrokers.map(function (connection) {
        return connection.send(correlationId, buffer).then(function (responseBuffer) {
            ...
            if (result.coordinators) {
                // V4+ response: extract first coordinator from array
...
Client.prototype.addOffsetsToTxnRequest = function (transactionalId, producerId, producerEpoch, groupId) {
    ...
    return self._findTransactionCoordinator(transactionalId).then(function (connection) {
        var correlationId = self.nextCorrelationId();
        var txnVersion = self._negotiateVersion(connection, globals.API_KEYS.AddOffsetsToTxnRequest, 3);
        var buffer, responseName;
...
Client.prototype.endTxnRequest = function (transactionalId, producerId, producerEpoch, committed) {
    ...
    return self._findTransactionCoordinator(transactionalId).then(function (connection) {
        var correlationId = self.nextCorrelationId();
        var txnVersion = self._negotiateVersion(connection, globals.API_KEYS.EndTxnRequest, 3);
        var buffer, responseName;
...
Client.prototype.txnOffsetCommitRequest = function (transactionalId, groupId, producerId, producerEpoch, offsets) {
    ...
    return self._findTransactionCoordinator(transactionalId).then(function (connection) {
        ...
        var topicGroups = Object.groupBy(offsets, function (o) { return o.topic; });
        ...
        var txnVersion = self._negotiateVersion(connection, globals.API_KEYS.TxnOffsetCommitRequest, 3);
        var buffer, responseName;
    ...
    if (nodeEndpoints && nodeEndpoints.length) {
        nodeEndpoints.forEach(function (ep) {
            ...
            if (!self.brokerConnections[ep.nodeId]) {
                ...
                self.apiVersionsRequest(self.brokerConnections[ep.nodeId]);
            }
        });
    }
