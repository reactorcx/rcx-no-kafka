...
Client.prototype._sendToAnyBroker = function (correlationId, buffer, parseResponse) {
    var self = this;
    var promises = self.initialBrokers.map(function (connection) {
        return connection.send(correlationId, buffer).then(function (responseBuffer) {
            return parseResponse(responseBuffer);
        });
    });

    // Manual "any" implementation — works with both native Promise and Bluebird
    var settled = false;
    var errorsList = [];
            ...
            Promise.resolve(p).then(function (result) {
                if (!settled) {
                    ...
                    self.initialBrokers.forEach(function (connection) {
                        connection.cancelPending(correlationId);
                    });
                    resolve(result);
                }
            }, function (err) {
