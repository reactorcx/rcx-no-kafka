...
// Grow geometrically rather than to an exact fit: a large response arrives in many
// small socket chunks, and exact-fit growth re-copies the whole accumulated buffer on
// every chunk, making reassembly O(n^2) in memcpy. That is negligible at a 1MB
// maxBytes but costs ~1s of blocked event loop for a 25MB response.
Connection.prototype._growBuffer = function (requiredLength) {
    var length = this.buffer.length > 0 ? this.buffer.length : requiredLength, _b;
    while (length < requiredLength) {
        length *= 2;
    }
    _b = Buffer.allocUnsafe(length);
