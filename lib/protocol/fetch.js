...
Protocol.define('FetchResponsePartitionItem', {
    read: function () {
        ...
        // a negative size would rewind the reader and desync the response parse
        ...
        if (this.context.messageSet.length === 1 && this.context.messageSet[0]._partial === true) {
            ...
            this.context.messageSet = utils.dropRightWhile(this.context.messageSet, { _partial: true }); // drop partially read messages
        }
    }
});
        ...
        if (this.context.messageSetSize > 0) {
            ...
            if (peekOffset < this.offset + this.context.messageSetSize && this.buffer[peekOffset] === 2) {
                ...
                if (this.context.messageSet.length === 1 && this.context.messageSet[0]._partial === true) {
                    ...
                    this.context.messageSet = utils.dropRightWhile(this.context.messageSet, { _partial: true });
                }
            }
        } else {
            this.context.messageSet = [];
...
// FetchResponseV8 through FetchResponseV10 — reuse FetchResponseV7 (identical wire format)
...
// FetchRequestV8 — identical wire format to v7 (version bump only), reuse FetchRequestV7
// FetchRequestV9 — adds currentLeaderEpoch per partition (v9 items defined above), otherwise same as v7
...
Protocol.define('FetchRequestV12PartitionItem', {
    write: function (data) { // {partition, currentLeaderEpoch, offset, lastFetchedEpoch, logStartOffset, maxBytes}
        this
            .Int32BE(data.partition)
            .Int32BE(data.currentLeaderEpoch !== undefined ? data.currentLeaderEpoch : -1)
            .KafkaOffset(data.offset)
            .Int32BE(data.lastFetchedEpoch !== undefined ? data.lastFetchedEpoch : -1)
            .KafkaOffset(data.logStartOffset || -1)
            .Int32BE(data.maxBytes)
            .TaggedFields();
    }
...
Protocol.define('FetchResponseV12AbortedTransaction', {
    read: function () {
        this
            .KafkaOffset('producerId')
            .KafkaOffset('firstOffset')
...
Protocol.define('FetchResponseV12PartitionItem', {
    read: function () {
        ...
        this
            ...
            .KafkaOffset('lastStableOffset')
            .KafkaOffset('logStartOffset')
            .compactArray('abortedTransactions', this.FetchResponseV12AbortedTransaction)
            .Int32BE('preferredReadReplica')
            .UVarint('messageSetSize');

        // compact encoding: N+1 where N is actual byte count (0 means null)
        if (this.context.messageSetSize > 0) {
            this.context.messageSetSize = this.context.messageSetSize - 1;
        }
        ...
        // Parse tagged fields — tag 1 = CurrentLeader (KIP-951)
        this.UVarint('_tagCount');
        for (i = 0; i < this.context._tagCount; i++) {
            this.UVarint('_tag');
            this.UVarint('_tagSize');
            if (this.context._tag === 1) {
                _tagStart = this.offset;
                this.ProduceResponseV10CurrentLeader('currentLeader');
                // skip unconsumed bytes if a future broker grows this tagged struct
                if ((this.offset - _tagStart) < this.context._tagSize) {
                    this.skip(this.context._tagSize - (this.offset - _tagStart));
                }
            } else {
...
Protocol.define('FetchRequestV14', {
    write: function (data) {
        this
            .FlexibleRequestHeader({
                apiKey: globals.API_KEYS.FetchRequest,
...
// v15 (KIP-903 Broker Epoch) — removes replicaId from request body
Protocol.define('FetchRequestV15', {
    write: function (data) {
        this
            .FlexibleRequestHeader({
                apiKey: globals.API_KEYS.FetchRequest,
...
// v16 (KIP-951 Leader Discovery) — request identical to v15; response adds NodeEndpoints (tag 0)
Protocol.define('FetchRequestV16', {
    write: function (data) {
        this
            .FlexibleRequestHeader({
                apiKey: globals.API_KEYS.FetchRequest,
...
Protocol.define('FetchResponseV16', {
    read: function () {
        var i, _tagStart;
        this
            .Int32BE('correlationId')
