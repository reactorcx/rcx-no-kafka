...
Protocol.define('DescribeGroupResponse_ConsumerGroupMemberItem', {
    read: function () {
        var _metaLength, _metaStart;
        this
            .string('memberId')
            .string('clientId')
            .string('clientHost')
            .Int32BE('_metaLength');
        _metaLength = this.context._metaLength;
        _metaStart = this.offset;
        if (_metaLength > 0) {
            this
                .Int16BE('version')
                .array('subscriptions', this.string)
                .bytes('metadata');
            // skip any unconsumed envelope bytes (newer subscription versions)
            if ((this.offset - _metaStart) < _metaLength) {
                this.skip(_metaLength - (this.offset - _metaStart));
            }
        }
        this.SyncConsumerGroupRequest_MemberAssignment('memberAssignment');
    }
});
...
// DescribeGroupResponseV2 — reuse DescribeGroupResponseV1 (identical wire format)
// ListGroupResponseV2 — reuse ListGroupResponseV1 (identical wire format)
...
Protocol.define('DescribeGroupResponseV5_ConsumerGroupMemberItem', {
    read: function () {
        ...
        if (this.context._metaLen > 0) {
            ...
            consumed = this.offset - _startOffset;
            if (consumed < n) {
