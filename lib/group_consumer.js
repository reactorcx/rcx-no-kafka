...
function GroupConsumer(options) {
    ...
    // Deadline shared by every drain in one heartbeat-suspended rebalance. See _withDrainBudget.
    this._drainDeadline = null;
...
GroupConsumer.prototype._rejoin = function () {
    ...
    // One budget for the whole rejoin: a cooperative phase-2 rejoin runs inside this call and
    // inherits it, so both drains together cannot outlast a single revokeTimeout.
    return self._withDrainBudget(function () {
...
/**
 * Run `fn` under one drain budget shared by every `_drainRevoked` it reaches.
 *
 * Heartbeats are suspended for the whole of a rebalance, and a cooperative rebalance can revoke in
 * two phases (`_needsRejoin` triggers a second `_rejoin`). Bounding each drain independently
 * therefore lets a single rebalance spend 2 x revokeTimeout with no heartbeat sent — and at the
 * default (`sessionTimeout / 2`) that is the entire session budget, before JoinGroup, SyncGroup,
 * updateMetadata and fetchOffset have had any of it. The broker evicts a member that has not
 * heartbeat within sessionTimeout, and the eviction triggers another rebalance, so bounding
 * per-drain is self-amplifying under exactly the conditions the drain exists for.
 *
 * Budgeting across the rejoin instead leaves the common single-phase case at its full window and
 * only shortens a second drain in the same rebalance. Nested calls inherit the budget; only the
 * call that opened it clears it.
 *
 * @param  {Function} fn
 * @return {Promise}
 */
GroupConsumer.prototype._withDrainBudget = function (fn) {
    var self = this, opened = false;

    if (self._drainDeadline === null) {
        self._drainDeadline = Date.now() + self.options.revokeTimeout;
        opened = true;
    }

    function release(result) {
        if (opened) { self._drainDeadline = null; }
        return result;
    }

    return Promise.resolve().then(fn).then(release, function (err) {
        release();
        throw err;
    });
};
...
GroupConsumer.prototype._drainRevoked = function (partitions, context) {
    var self = this, startedAt, bound;
    if (!partitions || partitions.length === 0 || !self._onPartitionsRevoked) {
        return Promise.resolve();
    }
    // Whatever is left of this rebalance's shared budget, or the full option when called outside
    // one (no rejoin path does, but _drainRevoked must not depend on that to stay correct).
    bound = self._drainDeadline === null ? self.options.revokeTimeout :
        Math.max(self._drainDeadline - Date.now(), 0);
    if (bound === 0) {
        // Either the explicit opt-out, or an earlier drain in this same rebalance already spent the
        // budget. Notify fire-and-forget (the pre-4.11 behaviour) and do not wait. Racing a 0ms
        // timer instead would fail the drain and warn about re-delivery the caller either accepted
        // or cannot avoid. The callback still runs: consumers use it to invalidate work in flight
        // for the revoked partitions, and skipping it outright would leave that undone.
        if (self.options.revokeTimeout !== 0) {
            self.client.warn('Drain budget for this rebalance is spent; notifying ' +
                'onPartitionsRevoked during ' + context + ' without awaiting it — these partitions ' +
                'may re-deliver');
        }
    ...
    startedAt = Date.now();
    return Promise.resolve().then(function () {
        return promiseUtils.timeout(self._onPartitionsRevoked(partitions), bound,
...
GroupConsumer.prototype._fullRejoin = function () {
    ...
    return (function _tryFullRejoin(attempt) {
        ...
        attempt = attempt || 0;
        // Each attempt is its own heartbeat-suspended unit — the retry backoff between them is
        // not — so it opens a fresh budget instead of inheriting the previous attempt's spent one.
        self._drainDeadline = Date.now() + self.options.revokeTimeout;
        ...
        // Stop fetching these before the drain, for the reason _updateSubscriptions states at its
        // own wipe: _fetch's handler-completion logic clears `paused` once an in-flight handler
        // resolves, so a revoked partition that happens to be idle keeps being polled for the whole
        // drain. Work fetched then is parked after the drain captured its entry list, so the drain
        // never sees it and it can commit for a partition this member no longer owns.
        // Deleted per owned partition rather than wiping self.subscriptions: in eager mode
        // ownedPartitions is never populated (only _updateSubscriptionsCooperative assigns it), so
        // flatPartitions is empty and this is a no-op — whereas a wipe would empty the very map the
        // eager drain derives previousPartitions from, silently disabling drain-on-rejoin there.
        flatPartitions.forEach(function (p) {
            delete self.subscriptions[p.topic + ':' + p.partition];
        });
