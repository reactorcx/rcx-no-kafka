...
GroupConsumer.prototype._fullRejoin = function () {
    ...
    return (function _tryFullRejoin(attempt) {
        ...
        attempt = attempt || 0;
        self.memberId = null;
        // AWAIT the revoke callback (drains in-flight, already-emitted-but-uncommitted work) for
        // whatever this instance still owned BEFORE wiping ownedPartitions below. Otherwise
        // _updateSubscriptionsCooperative's toRevoke diff (computed against ownedPartitions) is
        // always empty on a full rejoin, so the drain never runs — CORE-4239 residual gap: a full
        // rejoin (triggered when _rejoin's own bounded retries are exhausted, or a heartbeat
        // request itself fails) silently abandons in-flight work instead of draining it.
        return Promise.resolve().then(function () {
            var flatPartitions = [];
            if (!previouslyOwned || previouslyOwned.length === 0 || !self._onPartitionsRevoked) {
                return undefined;
            }
            ...
            return promiseUtils.timeout(
                self._onPartitionsRevoked(flatPartitions),
                self.options.revokeTimeout || Math.floor(self.options.sessionTimeout / 2),
                'onPartitionsRevoked drain timed out'
            );
        })
        .catch(function (err) {
            self.client.warn('onPartitionsRevoked drain failed during full rejoin, proceeding:', err);
        })
        .then(function () {
...
GroupConsumer.prototype._updateSubscriptions = function (partitionAssignment) {
    ...
    // AWAIT the revoke callback (it drains already-emitted-but-uncommitted in-flight work)
    // BEFORE re-subscribing, so fetchOffset reads the post-drain committed offset and the
    // rejoin cannot re-deliver that batch. Bounded by revokeTimeout so a callback that never
    // settles can't wedge the rebalance forever. A failing/timed-out drain degrades to the
    // previous behaviour (re-subscribe from the last committed offset). Evaluating the call
    // inside this .then (rather than as an argument to Promise.resolve) ensures a synchronous
    // throw from the callback is also caught below instead of escaping the chain.
    return Promise.resolve().then(function () {
        if (previousPartitions.length === 0 || !self._onPartitionsRevoked) { return undefined; }
        return promiseUtils.timeout(
            self._onPartitionsRevoked(previousPartitions),
            self.options.revokeTimeout || Math.floor(self.options.sessionTimeout / 2),
            'onPartitionsRevoked drain timed out'
        );
    })
    .catch(function (err) {
        self.client.warn('onPartitionsRevoked drain failed, proceeding with re-subscribe:', err);
    })
    .then(function () {
        self.subscriptions = {};
...
GroupConsumer.prototype._updateSubscriptionsCooperative = function (partitionAssignment, handler) {
    ...
    var revokeDrain = Promise.resolve();
    if (toRevoke.length > 0) {
        self.client.log('Cooperative rebalance: revoked', toRevoke.length, 'partitions');
        if (self._onPartitionsRevoked) {
            // See _updateSubscriptions for why this is bounded by revokeTimeout and why the
            // callback is invoked inside .then rather than as an argument to Promise.resolve.
            revokeDrain = Promise.resolve().then(function () {
                return promiseUtils.timeout(
            ...
            .catch(function (err) {
                self.client.warn('onPartitionsRevoked drain failed, proceeding:', err);
            });
        }
