...
function GroupConsumer(options) {
    this.options = utils.defaultsDeep(options || {}, {
        ...
        revokeTimeout: null, // bound on the onPartitionsRevoked drain, defaults to sessionTimeout / 2
...
GroupConsumer.prototype._drainRevoked = function (partitions, context) {
    var self = this, timeoutMs;
    if (!partitions || partitions.length === 0 || !self._onPartitionsRevoked) {
        return Promise.resolve();
    }
    // `|| default` would turn an explicit `revokeTimeout: 0` ("don't wait") into the default.
    timeoutMs = self.options.revokeTimeout === null || self.options.revokeTimeout === undefined ?
        Math.floor(self.options.sessionTimeout / 2) : self.options.revokeTimeout;
    // Invoked inside the .then so a synchronous throw from the callback is caught below too.
    return Promise.resolve().then(function () {
        return promiseUtils.timeout(self._onPartitionsRevoked(partitions), timeoutMs,
