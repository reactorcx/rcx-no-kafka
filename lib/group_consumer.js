...
function GroupConsumer(options) {
    ...
    // Derived from sessionTimeout, so it cannot be expressed in the defaults literal above.
    // Resolved here rather than per-rebalance, and an explicit 0 ("don't wait") is preserved.
    ...
    if (this.options.revokeTimeout >= this.options.sessionTimeout) {
