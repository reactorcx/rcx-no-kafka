...
function GroupConsumer(options) {
    ...
    if (this.options.revokeTimeout === null || this.options.revokeTimeout === undefined) {
        ...
        this.options.revokeTimeout = Math.floor(this.options.sessionTimeout / 2);
    } else if (this.options.revokeTimeout >= this.options.sessionTimeout) {
        // Heartbeats are suspended for the whole rebalance, drain included, so a revokeTimeout at
        // or above sessionTimeout guarantees the member is evicted whenever the drain runs long.
        this.client.warn('revokeTimeout (' + this.options.revokeTimeout + 'ms) is >= sessionTimeout (' +
            this.options.sessionTimeout + 'ms); a slow onPartitionsRevoked drain will get this member evicted');
    }
...
GroupConsumer.prototype._drainRevoked = function (partitions, context) {
    ...
    .catch(function (err) {
        self.client.warn('onPartitionsRevoked drain failed during ' + context + ' for ' +
...
GroupConsumer.prototype._updateSubscriptions = function (partitionAssignment) {
    ...
    .then(function () {
        // Drop anything a concurrent subscribe() landed while the drain was running: until the
        // new assignment is subscribed below, this consumer owns nothing.
        self.subscriptions = {};
...
GroupConsumer.prototype._updateSubscriptionsCooperative = function (partitionAssignment, handler) {
    ...
    return self._drainRevoked(toRevoke, 'cooperative rebalance').then(function () {
        // Re-drop the revoked partitions, mirroring the eager path: a subscribe() already past
        // its guard when unsubscribe() ran above lands after the delete, and nothing else here
        // removes it — the partition would keep being fetched after we have given it up, while
        // its new owner consumes it too. The awaited drain widens that window to revokeTimeout.
        toRevoke.forEach(function (r) {
            self.unsubscribe(r.topic, r.partition);
        });
