...
GroupConsumer.prototype._updateSubscriptions = function (partitionAssignment) {
    ...
    // Pause the partitions being given up so the fetch loop stops delivering new work
    // while we drain their in-flight commits below.
    ...
    // BEFORE wiping and re-subscribing, so fetchOffset reads the post-drain committed offset
    // and the rejoin cannot re-deliver that batch. A failing drain degrades to the previous
    // behaviour (re-subscribe from the last committed offset).
    return Promise.resolve(
        (previousPartitions.length > 0 && self._onPartitionsRevoked) ?
            self._onPartitionsRevoked(previousPartitions) : undefined
    )
    .catch(function (err) {
...
GroupConsumer.prototype._updateSubscriptionsCooperative = function (partitionAssignment, handler) {
    ...
    if (toRevoke.length > 0) {
        ...
        if (self._onPartitionsRevoked) {
            revokeDrain = Promise.resolve(self._onPartitionsRevoked(toRevoke)).catch(function (err) {
