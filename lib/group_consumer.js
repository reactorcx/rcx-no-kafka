...
GroupConsumer.prototype._syncGroup = function () {
    ...
    return Promise.resolve().then(function () {
        ...
        if (self.memberId === self.leaderId) { // leader should generate group assignments
            // the leader must resolve metadata for ALL members' subscriptions,
            // not just its own topics
            ...
            return self.client.updateMetadata(Object.keys(topicMap)).then(function () {
                var r = [], topicKeys, k;
                topicKeys = Object.keys(topicMap);
                for (k = 0; k < topicKeys.length; k++) {
                    if (!self.client.topicMetadata[topicKeys[k]]) {
                        // skip topics that don't exist instead of failing the whole
                        // sync (one member's bad subscription must not brick the leader)
                        self.client.error('Sync group: unknown topic:', topicKeys[k]);
                        continue;
                    }
                    r.push({
                        topic: topicKeys[k],
                        members: topicMap[topicKeys[k]],
                        partitions: Object.values(self.client.topicMetadata[topicKeys[k]]).map(function (p) { return p.partitionId; })
                    });
                }
...
GroupConsumer.prototype._fullRejoin = function () {
    ...
    .then(function (r) {
        if (self._closed) {
            // end() raced the rejoin — leave so the member doesn't squat in the
            // group until the session timeout (static members keep their slot)
            if (self.memberId && !self.options.groupInstanceId) {
                self.client.leaveGroupRequest(self.options.groupId, self.memberId).catch(function () { return null; });
            }
            return r;
        }
        self._heartbeatPromise = self._runHeartbeat(); // start sending heartbeats
        return r;
    });
};
 ...
 * process on modern Node. Terminal failures (e.g. FencedInstanceId) end the loop.
 */
GroupConsumer.prototype._runHeartbeat = function () {
    var self = this;
    return self._heartbeat().catch(function (err) {
        self.client.error('Heartbeat loop terminated:', err);
    });
};
...
GroupConsumer.prototype._updateSubscriptions = function (partitionAssignment) {
    ...
    if (previousPartitions.length > 0 && self._onPartitionsRevoked) {
