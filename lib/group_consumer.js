...
/**
 * AWAITS the `onPartitionsRevoked` callback for `partitions` — draining in-flight,
 * already-emitted-but-uncommitted work — bounded by revokeTimeout so a callback that never
 * settles can't wedge a rebalance forever. A failing/timed-out drain degrades to the previous
 * fire-and-forget behaviour (proceed regardless, having warned) rather than ever hanging or
 * throwing out of a rebalance path. Shared by every rebalance path that revokes partitions
 * (`_fullRejoin`, `_updateSubscriptions`, `_updateSubscriptionsCooperative`) so the guard/bound/
 * degrade behaviour lives in one place instead of being re-derived at each call site.
 *
 * @param  {Array}  partitions   flat [{topic, partition}, ...] being revoked
 * @param  {String} warnMessage  logged (with the error) if the drain fails or times out
 * @return {Promise} always resolves — never rejects
 */
GroupConsumer.prototype._drainRevoked = function (partitions, warnMessage) {
    var self = this;
    if (!partitions || partitions.length === 0 || !self._onPartitionsRevoked) {
        return Promise.resolve();
    }
    // Invoking the callback inside this .then (rather than as an argument to Promise.resolve)
    // ensures a synchronous throw from the callback is also caught by the .catch below.
    return Promise.resolve().then(function () {
        return promiseUtils.timeout(
            self._onPartitionsRevoked(partitions),
            self.options.revokeTimeout || Math.floor(self.options.sessionTimeout / 2),
            'onPartitionsRevoked drain timed out'
        );
    })
    .catch(function (err) {
        self.client.warn(warnMessage, err);
    });
};
...
GroupConsumer.prototype._fullRejoin = function () {
    ...
    return (function _tryFullRejoin(attempt) {
        var retryDelay;
        var previouslyOwned = self.ownedPartitions;
        var flatPartitions = [];
        attempt = attempt || 0;
        self.memberId = null;
        // Drain whatever this instance still owned BEFORE wiping ownedPartitions below.
        // Otherwise _updateSubscriptionsCooperative's toRevoke diff (computed against
        // ownedPartitions) is always empty on a full rejoin, so the drain never runs —
        // CORE-4239 residual gap: a full rejoin (triggered when _rejoin's own bounded retries
        // are exhausted, or a heartbeat request itself fails) silently abandons in-flight work
        // instead of draining it.
        if (previouslyOwned) {
            previouslyOwned.forEach(function (tp) {
                tp.partitions.forEach(function (p) {
                    flatPartitions.push({ topic: tp.topic, partition: p });
                });
            });
        }
        return self._drainRevoked(flatPartitions, 'onPartitionsRevoked drain failed during full rejoin, proceeding:')
        .then(function () {
...
GroupConsumer.prototype._updateSubscriptions = function (partitionAssignment) {
    ...
    // Remove the revoked partitions from the fetch loop now (hard delete, same as
    // unsubscribe()) rather than merely pausing them: _fetch's own handler-completion logic
    // (base_consumer.js) unconditionally clears `paused` once an in-flight handler resolves,
    // which would silently re-enable fetching for a partition mid-drain and re-deliver a
    // batch the drain below never saw. A deleted entry can't be revived that way — _fetch
    // only looks at keys still present in self.subscriptions.
    previousPartitions.forEach(function (p) {
        delete self.subscriptions[p.topic + ':' + p.partition];
    });

    // AWAIT the revoke drain (already-emitted-but-uncommitted in-flight work) BEFORE
    // re-subscribing, so fetchOffset reads the post-drain committed offset and the rejoin
    // cannot re-deliver that batch. A failing/timed-out drain degrades to the previous
    // behaviour (re-subscribe from the last committed offset).
    return self._drainRevoked(previousPartitions, 'onPartitionsRevoked drain failed, proceeding with re-subscribe:')
    .then(function () {
        self.subscriptions = {};
...
GroupConsumer.prototype._updateSubscriptionsCooperative = function (partitionAssignment, handler) {
    ...
    // AWAIT the revoke drain (in-flight commits for the revoked partitions) before
    // subscribing the newly-added partitions, so a partition given up here is committed
    // past its in-flight batch before it is re-fetched (by the new owner or on re-acquire).
    // A failing drain degrades to the previous behaviour.
    var revokeDrain = self._drainRevoked(toRevoke, 'onPartitionsRevoked drain failed, proceeding:');

    // Update ownedPartitions to match new assignment
    self.ownedPartitions = partitionAssignment;

    return revokeDrain.then(function () {
        if (toAdd.length === 0) {
