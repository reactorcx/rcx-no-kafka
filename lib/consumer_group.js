...
function ConsumerGroup(options) {
    ...
    // last target assignment from the coordinator ([{topicId, partitions}] or null).
    // The coordinator only sends the assignment when it CHANGES, so the target is
    // kept and re-reconciled on every heartbeat until the owned set matches it
    // (a transient subscribe/metadata failure would otherwise stall forever).
    this._targetAssignment = null;
    ...
    return (function _tryJoin(attempt) {
        ...
        .then(function (response) {
            if (self._closed) {
                ...
                return self.client.consumerGroupHeartbeatRequest(
                    self.options.groupId, response.memberId || self.memberId, -1, null, null, null
...
/**
 * Run one heartbeat with a terminal-failure guard: in timer/loop context there is
 * no caller to receive a rejection, and an unhandled rejection kills the process
 * on modern Node. Terminal failures (e.g. join attempts exhausted) end the loop.
 */
ConsumerGroup.prototype._runHeartbeat = function () {
    var self = this;
    return self._heartbeat().catch(function (err) {
        self.client.error('Consumer group heartbeat loop terminated:', err);
    });
};
...
ConsumerGroup.prototype._maybeReconcile = function (assignment) {
    ...
    if (assignment) {
        self._targetAssignment = assignment.topicPartitions || [];
    }

    if (!self._targetAssignment) {
        return Promise.resolve(); // nothing assigned yet
    }

    // _reconcile is idempotent: when owned already matches the target it no-ops,
    // otherwise it retries whatever revoke/acquire is still outstanding
    return self._reconcile(self._targetAssignment);
};
...
ConsumerGroup.prototype._revokeAll = function () {
    ...
    self._targetAssignment = null; // a fresh assignment arrives with the rejoin
...
ConsumerGroup.prototype.commitOffset = function (commits) {
    ...
    if (self.memberEpoch <= 0) {
        // not currently a group member (joining, or fenced and rejoining)
