...
* `revokeTimeout` - maximum time in ms to wait for the `onPartitionsRevoked` callback to settle before continuing the rebalance without it, defaults to half of `sessionTimeout`. `0` means notify but do not wait (the callback still runs, nothing is awaited). Invalid values fall back to the default with a warning. Heartbeats are suspended for the duration of the wait, so a value at or above `sessionTimeout` will get the member evicted whenever the drain runs long; that case is warned about at construction
