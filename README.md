...
* `revokeTimeout` - maximum time in ms to wait for the `onPartitionsRevoked` callback to settle before continuing the rebalance without it, defaults to half of `sessionTimeout`. `0` means notify but do not wait (the callback still runs, nothing is awaited). Invalid values fall back to the default with a warning. Heartbeats are suspended for the duration of the wait and the drain is only one step of a rebalance, so a value above half of `sessionTimeout` leaves little budget for JoinGroup/SyncGroup/fetchOffset and risks eviction; that case is warned about at construction
