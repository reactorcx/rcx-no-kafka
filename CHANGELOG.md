...
### Added
- `revokeTimeout` — bound in ms on the `onPartitionsRevoked` wait, defaults to half of `sessionTimeout`. If the callback rejects or exceeds this, a warning naming the affected partitions is logged and the rebalance continues from the last committed offset (the pre-4.11 behaviour). An explicit `0` means "notify but do not wait" — the callback is still invoked and nothing is awaited; the drain is disabled, which is warned about once at construction. Invalid values (non-numeric, negative, NaN) are warned about and fall back to the default rather than silently disabling the drain. A value above half of `sessionTimeout` is warned about at construction: heartbeats are suspended for the whole rebalance and the drain is only one step of it, so past that point a slow drain crowds out JoinGroup/SyncGroup/fetchOffset. The drain reduces re-delivery but does not eliminate it — handlers must stay idempotent.
