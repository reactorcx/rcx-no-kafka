...
### Added
- `revokeTimeout` — bound in ms on the `onPartitionsRevoked` wait, defaults to half of `sessionTimeout`. If the callback rejects or exceeds this, a warning naming the affected partitions is logged and the rebalance continues from the last committed offset (the pre-4.11 behaviour). An explicit `0` means "notify but do not wait" — the callback is still invoked, nothing is awaited, and nothing is warned about. Invalid values (non-numeric, negative, NaN) are warned about and fall back to the default rather than silently disabling the drain. A value at or above `sessionTimeout` is warned about at construction, since heartbeats are suspended for the duration of the drain.
